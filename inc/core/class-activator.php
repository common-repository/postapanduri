...
class Activator
{
    ...
    public static function activate()
    {
        ...
        if (\version_compare(PHP_VERSION, $php_min_version, '<')) {
            ...
            wp_die('This plugin requires a minimum PHP Version of ' . $php_min_version);
        }
        ...
        if (\version_compare(\curl_version()['version'], $curl_min_version, '<')) {
            deactivate_plugins(NS\PLUGIN_BASENAME);
            wp_die('This plugin requires a minimum cURL Version of ' . $curl_min_version);
        }

        if (!\extension_loaded('openssl')) {
            deactivate_plugins(NS\PLUGIN_BASENAME);
            wp_die('This plugin requires a minimum OpenSSL extension');
        }

        if (OPENSSL_VERSION_NUMBER < $openssl_min_version) {
            deactivate_plugins(NS\PLUGIN_BASENAME);
            wp_die('This plugin requires a minimum OpenSSL Version of 1.0.1' . $openssl_min_version);
        }
    ...
    public static function install_db()
    {
        ...
        if ($installed_ver < $jal_db_version) {
            ...
            try {
                ...
                if ($installed_ver < 2.0) {
                    $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}lo_dp_day_exceptions;");
                }
                ...
                if ($installed_ver < 2.0) {
                    $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}lo_dp_program;");
                }
                ...
                $table_name = $wpdb->prefix . 'lo_locker_push';
                $sql = "CREATE TABLE $table_name (
        ...
        if (!wp_next_scheduled('postapanduri_generate_sitemaps_event')) {
            wp_schedule_event(time(), 'weekly', 'postapanduri_generate_sitemaps_event');
        }
    }
}
