...
class WC_PostaPanduri extends \WC_Shipping_Method
{
    ...
    public $icon;
    public $pp_denied_payment_methods;
    public function __construct($instance_id = 0)
    {
        parent::__construct();
        self::$pp_order_statuses = array(
            'wc-pp-alocata' => array('denumire' => __('PostaPanduri - Assigned to courier', 'postapanduri'), 'cod' => 50),
            'wc-pp-preluata' => array('denumire' => __('PostaPanduri - Taken by courier from the merchant', 'postapanduri'), 'cod' => 100),
            'wc-pp-negasit' => array('denumire' => __('PostaPanduri - The recipient was not found', 'postapanduri'), 'cod' => 250),
            'wc-pp-predata-sl' => array('denumire' => __('PostaPanduri - Delivered in Smart Locker', 'postapanduri'), 'cod' => 290),
            'wc-pp-livrata' => array('denumire' => __('PostaPanduri - Delivered to the recipient', 'postapanduri'), 'cod' => 300),
            'wc-pp-amanata' => array('denumire' => __('PostaPanduri - Delivery delayed', 'postapanduri'), 'cod' => 350),
            'wc-pp-gresita' => array('denumire' => __('PostaPanduri - Wrong destination', 'postapanduri'), 'cod' => 500),
            'wc-pp-in-anulare' => array('denumire' => __('PostaPanduri - Pending cancel', 'postapanduri'), 'cod' => 550),
            'wc-pp-anulata' => array('denumire' => __('PostaPanduri - Canceled', 'postapanduri'), 'cod' => 600),
            'wc-pp-retur' => array('denumire' => __('PostaPanduri - Return', 'postapanduri'), 'cod' => 650),
            'wc-pp-ramburs-achitat' => array('denumire' => __('PostaPanduri - Paid for on delivery', 'postapanduri'), 'cod' => 850),
            'wc-pp-facturata' => array('denumire' => __('PostaPanduri - Invoiced', 'postapanduri'), 'cod' => 900),
            'wc-pp-finalizata' => array('denumire' => __('PostaPanduri - Completed', 'postapanduri'), 'cod' => 1000),
        );
        ...
        $this->icon = NS\PLUGIN_NAME_URL . 'assets/img/logo.png';
    ...
    {
        return $order->get_meta('id_pachetomat');
    }
    ...
    public static function recalculate_shipping_cost($serviciuid, $pret, $tip)
    {
        ...
        // verific daca iau in considerare subtotalul sau totalul comenzii
        $free_shipping_calculation_method = empty(get_option('postapanduri_setari_generale')['free_shipping_calculation_method']) ? 'subtotal' : get_option('postapanduri_setari_generale')['free_shipping_calculation_method'];

        switch ($free_shipping_calculation_method) {
            case 'subtotal_with_coupons':
                $amount = WC()->cart->get_cart_contents_total() + WC()->cart->get_cart_contents_tax();
                break;
            default:
        ...
        if (isset($serviciu->gratuit_peste) && $serviciu->gratuit_peste) {
            if ((float)$amount > (float)$serviciu->gratuit_peste) {
                return 0;
            }
        }
        ...
        if (\wc_string_to_bool(get_option('woocommerce_calc_taxes'))) {
            ...
            if (!empty($shipping_rate)) {
                ...
                return $pret ? \WC_TAX::round($pret / (1 + $shipping_rate['rate'] / 100), 2) : 0;
            }
            return $pret ? \WC_TAX::round($pret, 2) : 0;
        } else {
            return $pret ? \WC_TAX::round($pret, 2) : 0;
        }
    }
...
//
        ...
        $chosen_method = isset(WC()->session->get('chosen_shipping_methods')[0]) ? WC()->session->get('chosen_shipping_methods')[0] : '';
        $method = isset(explode('_', $chosen_method)[0]) ? explode('_', $chosen_method)[0] : '';
    ...
    private function estimeazaPretServicii($id_shipping_company, $package)
    {
        ...
        foreach ($package['contents'] as $item_id => $values) {
            ...
            $_product_weight = ($_product->has_weight() ? $_product->get_weight() : 0);
            $greutate += $_product_weight * $_product_quantity;
        }
        if ($greutate == 0) {
            $greutate = self::convert_weight_from_kg(get_option('woocommerce_weight_unit'), 1);
        }
        $greutate = round(self::convert_weight_to_kg(get_option('woocommerce_weight_unit'), $greutate), 2);
    ...
    public static function convert_weight_to_kg($unit, $value)
    {
        switch ($unit) {
            case 'g':
                return $value / 1000;
    ...
    public static function convert_weight_from_kg($unit, $value)
    {
        switch ($unit) {
            case 'g':
                return $value * 1000;
