...
use PostaPanduri\Inc\Libraries\phpseclib\Crypt\AES;
use PostaPanduri\Inc\Libraries\phpseclib\Crypt\RSA;
...
class LO
{
	...
	private static $url_cancel_livrare = 'https://api.livrarionline.ro/Lobackend.asmx/CancelLivrare';
	private static $url_returnare_livrare = 'https://api.livrarionline.ro/Lobackend.asmx/ReturnareLivrare';
	private static $url_generare_awb = 'https://api.livrarionline.ro/Lobackend.asmx/GenerateAwb';
	private static $url_register_awb = 'https://api.livrarionline.ro/Lobackend.asmx/RegisterAwb';
	private static $url_tracking_awb = 'https://api.livrarionline.ro/Lobackend.asmx/Tracking';
	private static $url_estimare_pret = 'https://estimare.livrarionline.ro/EstimarePret.asmx/EstimeazaPret';
	private static $url_estimare_pret_servicii = 'https://estimare.livrarionline.ro/EstimarePret.asmx/EstimeazaPretServicii';
	private static $url_locker_expectedin = 'https://smartlocker.livrarionline.ro/api/GetLockerExpectedInID';
	private static $url_cancel_locker_expectedin = 'https://smartlocker.livrarionline.ro/api/CancelLockerExpectedInID';
	private static $url_get_pachetomate_pr = 'https://api.livrarionline.ro/Lobackend.asmx/GetPachetomatePR';
	private static $url_set_pachetomat_default = 'https://api.livrarionline.ro/Lobackend.asmx/SetPachetomatDefault';
	...
	{
		return $this->LOCommunicate($f_request, self::$url_set_pachetomat_default);
	}

	public function GetPachetomatePR($f_request)
	{
		return $this->LOCommunicate($f_request, self::$url_get_pachetomate_pr);
	}

	public function CancelLivrare($f_request)
	{
		return $this->LOCommunicate($f_request, self::$url_cancel_livrare);
	}

	public function ReturnareLivrare($f_request)
	{
		return $this->LOCommunicate($f_request, self::$url_returnare_livrare);
	}

	public function GenerateAwb($f_request)
	{
		return $this->LOCommunicate($f_request, self::$url_generare_awb);
	}
	...
	public function GenerateAwbSmartloker($f_request, $delivery_point_id, $marime_celula, $order_id)
	{
		...
		$sql = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}lo_delivery_points where dp_id = %d", $delivery_point_id);
	...
	{
		return $this->LOCommunicate($f_request, self::$url_register_awb);
	}
	...
	{
		return $this->LOCommunicate($f_request, self::$url_tracking_awb);
	}

	public function EstimeazaPret($f_request)
	{
		return $this->LOCommunicate($f_request, self::$url_estimare_pret);
	}

	public function EstimeazaPretServicii($f_request)
	{
		return $this->LOCommunicate($f_request, self::$url_estimare_pret_servicii);
	}
	...
	{
		return $this->LOCommunicate($f_request, self::$url_locker_expectedin, true);
	}

	public function cancelExpectedIn($f_request)
	{
		return $this->LOCommunicate($f_request, self::$url_cancel_locker_expectedin, true);
	}
	...
	public function get_count_delivery_points_by_type()
	{
		global $wpdb;
		$sql = "SELECT
				    dp_tip, count(dp_id) as cate
				FROM
	...
	public function get_all_delivery_points_states($dp_tip = false)
	{
		global $wpdb;
		...
		$sql = "SELECT
			...
			($dp_tip !== false ? ' and dp_tip = ' . (int)$dp_tip : '') . "
				order by
				    dp.dp_judet asc
				";
	...
	public function get_all_delivery_points_location_by_state($judet, $dp_tip = false)
	{
		global $wpdb;
		$sql = "SELECT
				    distinct dp_oras
	...
	public function get_all_delivery_points_location_by_judet($judet = '', $dp_tip = false, $active_only = true)
	{
		global $wpdb;
		$sql = "SELECT
					dp.*,
				...
				WHERE
					" . ($active_only ? ' dp_active > 0' : ' dp_active >= 0') . "
	...
	public function get_statistics()
	{
		global $wpdb;
		$sql = "SELECT
					count(distinct dp_oras) as nr_localitati,
       				count(dp_id) as nr_pachetomate,
       				sum(case when lower(dp_denumire) like '%Statie Postala Digitala%' then 1 else 0 end) as nr_statii_postale
				FROM
	...
	public function get_all_delivery_points_select_pachetomate()
	{
		global $wpdb;
		$sql = "SELECT
					dp_judet, dp_oras,
			        CONCAT('[',GROUP_CONCAT(CONCAT('{\"dp_id\":\"', dp_id, '\", \"dp_denumire\":\"',dp_denumire,'\"}')),']') as pachetomate
				FROM
					{$wpdb->prefix}lo_delivery_points
				WHERE
					dp_active > 0 and dp_tip = 1
				group by
					dp_judet, dp_oras
				order by
				   dp_judet asc, dp_oras asc
				";
		return $wpdb->get_results($sql);
	}

	public function get_all_delivery_points_location_by_localitate($judet = '', $oras = '', $dp_tip = false)
	{
		global $wpdb;
		$sql = "SELECT
					dp.*,
	...
	public function get_all_delivery_points_location_admin()
	{
		global $wpdb;
		$sql = "SELECT
					dp.*,
				...
				group by
					dp.dp_id
				order by
				    dp_tip desc, dp_judet asc, dp_oras asc, dp.dp_denumire asc, dp.dp_active asc
				";
